...
import (
	"fmt"
	"zk-rawdata-reader/vzReader/models"
)

/* GetHTTPRawData returns the raw data for HTTP for the given traceIds and startTime */
func (config *VzReader) GetHTTPRawData(traceIds []string, startTime string) (*models.PixieHTTPResponse[models.HTTP_raw_data], error) {
	...
	response := models.PixieResponseToHTTPResponse[models.HTTP_raw_data](result, tm)
	return response, nil
}
